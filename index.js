...
app.post("/prebill",async(req,res)=>{
    ...
    if(choice==1){
        ...
        res.render("prebill_air_ticket.ejs",{bill_id:currentbillno,
            customer_name:currentbillername.rows[0].customer_name,
            air_ticket:current_air_tickets.rows});
        ...
        res.render("prebill_hotel.ejs",{bill_id:currentbillno,
            customer_name:currentbillername.rows[0].customer_name,
            hotel_booking:current_hotel_booking.rows});
    }
})

app.post("/bill_hotel",async (req,res)=>{
    let selected_hotel_booking=[];
    
    for(let j=0;j<req.body.hotel_booking.length;j++){
        let ticket=parseInt(req.body.hotel_booking[j]);
        selected_hotel_booking.push(ticket);
    };
    selected_hotel_booking.forEach((ticket)=>{
        ticket=parseInt(ticket);
        let query_2345=db.query("UPDATE hotel_booking SET hotel_booking_biller_id=$1,hotel_booking_bill_id=$2 WHERE hotel_booking_id=$3",[currentbillerid,currentbillno,ticket]);
    })
    let query1="SELECT * FROM hotel_booking WHERE hotel_booking_id IN";
    let where_value1="";
    for(let i=0;i<=selected_hotel_booking.length+1;i++){
        if(i==0){
            where_value1+="(";
        }
        else if(i==selected_hotel_booking.length+1){
            where_value1+=");";
        }
        else{
            if(i==selected_hotel_booking.length){
                where_value1+="$"+i;
            }
            else{
                where_value1+="$"+i+",";
    ...
    query1+=where_value1;
    let final_hotel_booking= await db.query(`${query1}`,selected_hotel_booking);
    


    let current_customer=await db.query("SELECT * FROM customer_info WHERE customer_id=$1",[currentbillerid]);
    res.render("bill_hotel.ejs",{hotel_booking:final_hotel_booking.rows,
        customer:current_customer.rows[0],sno:1,billno:currentbillno});
});
...
app.post("/bill_air",async(req,res)=>{
    ...
    for(let j=0;j<req.body.air_tickets.length;j++){
        let ticket=parseInt(req.body.air_tickets[j]);
    ...
    res.render("bill_air_ticket.ejs",{air_ticket:final_air_ticket.rows,
            customer:current_customer.rows[0],sno:1,billno:currentbillno,date:date,service_charge:service_charge,service_charge_quantity:service_charge_quantity,service_charge_rate:service_charge_rate});
})
